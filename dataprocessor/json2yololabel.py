        ...
        with open(jsonfile, "r") as f:
            ...
            with open(resultDirPath + "/" + jsonfile.split("/")[-1].replace(".json", ".txt"), "w") as file_handle:
                ...
                for shape in shapes:
                    ...
                    file_handle.writelines(str(classList.index(shape["label"])) + " ") # 类别不变
                    #file_handle.writelines(str(0) + " ") # 变为1类-----------------------------------------
                    # 8. 遍历shape轮廓中每个点，每个点要进行缩放，即x/width, y/height
...
if __name__ == "__main__":
    jsonfilePath = "/home/disk/YC/Projects/yolov8/segment/workspace/data/VAND_ASC/upper_train/json"  # 要转换的json文件所在目录
    resultDirPath = os.path.join(os.path.dirname(jsonfilePath),"labels")
    labelme2yolov2Seg(jsonfilePath=jsonfilePath, resultDirPath=resultDirPath, classList=["dasuixie", "suixie", "yiwu","fanzhe"])
